...
neighbour_distance.scaled = function(i, reduction, seu) {
  # extract neighbour cells of cell i
  n = colnames(seu@graphs$RNA_nn)[seu@graphs$RNA_nn[i,] == 1] # please change "RNA_nn" to "SCT_nn" if you are using SCTranform
...
calculate_neighbour_distance_for_all_cells <- function(seu, reduction, colname) {
  for(i in 1:nrow(seu@meta.data)) {seu@meta.data[[colname]][i] = neighbour_distance.scaled(i, reduction, seu)}
}
